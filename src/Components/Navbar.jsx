...
const Navbar = () => {
  ...
  return (
    <nav className="fixed w-full backdrop-blur-sm bg-transparent dark:bg-gray-900 shadow-sm z-50">
          ...
          <div className="hidden md:flex items-center space-x-6">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-gray-700 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-gray-700 rounded-3xl px-3 py-2 text-sm font-medium transition-colors"
              >
                {item.name}
              </a>
            ))}
